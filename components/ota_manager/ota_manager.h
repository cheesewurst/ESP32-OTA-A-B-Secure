...
typedef struct {
    ...
    char                    update_url[256]; // URL for OTA updates
} ota_manager_info_t;
...
esp_err_t ota_manager_check_https_url(const char* url); // Check if the provided URL is valid and accessible
...
esp_err_t ota_manager_get_update_status(ota_update_status_t* update_status); // Get the current status of the OTA update process
